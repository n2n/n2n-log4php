<?php
namespace n2n\log4php\configuration\adapter;
/**
...
class AdapterPHP implements \n2n\log4php\configuration\ConfigurationAdapter 
{
	public function convert($url) {
		...
		if ($config === false) {
			$error = error_get_last();
			throw new \n2n\log4php\LoggerException("Error parsing configuration: " . $error['message']);
		}
		
		if (empty($config)) {
			throw new \n2n\log4php\LoggerException("Invalid configuration: empty configuration array.");
		}
		
		if (!is_array($config)) {
			throw new \n2n\log4php\LoggerException("Invalid configuration: not an array.");
		}
